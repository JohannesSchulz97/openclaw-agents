...
# Workspace blocker - reminds to use openclaw-workspace skill for .openclaw file ops
...
# Only process ~/.openclaw file operations
case "$COMMAND" in
    *~/.openclaw*|*.openclaw*)
        ...
        if [[ $should_block -eq 1 ]]; then
            # Deny with reminder
            jq -n \
                --arg msg "Reminder: For agent workspace management, use openclaw-workspace skill. See .claude/skills/openclaw-workspace/SKILL.md" \
                '{permissionDecision: "deny", additionalContext: $msg}'
            exit 0
        else
            # Allow but inject compact checklist
            jq -n \
                --arg msg ".openclaw: Use openclaw-workspace skill for file operations." \
