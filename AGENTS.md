...
- **PreToolUse hooks** — hard-block edits to `.openclaw/agents/*/` shared files and scripts (must use `types/` instead), block `deploy.yml` edits without approval, and fire-once deny on `jobs-config.json`, type scripts, workspace config files, and `AGENTS.md` to force reading the relevant skill first.
